...
class AuthController extends Controller
{
    ...
    public function loginAdministrador(LoginRequest $request): JsonResponse {
        ...
        $ip = explode(',', $request->header('X-Forwarded-For'))[0] ?? $request->ip(); // Obtenemos la IP del usuario que realiza la petición, si hay proxy obtenemos la auténtica con X-Forwarded-For
        ...
        $ubicacionAPI = file_get_contents("http://ip-api.com/json/{$ip}"); // Hacemos petición de geolocalización a la API. Esta sería la versión a poner
